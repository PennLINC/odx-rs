...
impl MifImage {
    ...
    pub fn element_size(&self) -> usize {
        match self.header.datatype.as_str() {
    ...
    pub fn affine_4x4(&self) -> [[f64; 4]; 4] {
        if let Some(ref t) = self.header.transform {
            let vx = self.header.voxel_sizes.first().copied().unwrap_or(1.0);
            let vy = self.header.voxel_sizes.get(1).copied().unwrap_or(1.0);
            let vz = self.header.voxel_sizes.get(2).copied().unwrap_or(1.0);
            // MRtrix `layout` describes on-disk storage order only. The image
            // has already been reordered into logical voxel order using that
            // layout, so the affine must come from `transform` and `vox`
            // alone; applying layout sign flips here would mirror the volume.
            [
            ...
            let vs = &self.header.voxel_sizes;
